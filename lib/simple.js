...
async value(jid, text = '', quoted, options) {
    if (Buffer.isBuffer(text)) {
        ...
        let canalId = [
            ...
            // أضف المزيد من الروابط إذا لزم الأمر
        ];
        let canalNombre = ["majnon tech"];
        ...
        // استخراج المعرف من الرابط إذا لزم الأمر
        let channelId = randomChannel.id.split('/').pop(); // استخراج المعرف من الرابط
        const contextInfo = {
            mentionedJid: await conn.parseMention(text),
            isForwarded: true,
            forwardingScore: 1, 
            forwardedNewsletterMessageInfo: {
                newsletterJid: channelId, // استخدام المعرف بدلاً من الرابط
                newsletterName: randomChannel.nombre,
                serverMessageId: 100
            }
        };

        // هنا يمكنك استخدام contextInfo كما تحتاج
    }
}
