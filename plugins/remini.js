...
async function handler(m, { conn, usedPrefix, command }) {
    try {
        ...
        if (/^image/.test(mime) && !/webp/.test(mime)) {
            ...
            const api = await fetch(`https://api.betabotz.eu.org/api/tools/remini?url=${out}&apikey=ZnGvIADT`);
            const image = await api.json();
            const { url } = image;
            conn.sendFile(m.chat, url, null, 'https://whatsapp.com/channel/0029VaiJnhbD38CbP5YcSK0K', m);
        } else {
